...
export default function FeaturedServices() {
    ...
    const fetchFeaturedServices = async () => {
        try {
            const response = await fetch('http://localhost:5000/api/services?featured=true&status=active');
            if (!response.ok) {
            ...
            setServices(transformedServices.slice(0, 12)); // Limit to 12 featured services
        } catch (error) {
        ...
        return (
            <div className="bg-[#F8F9FA] py-12 px-4">
                <div className="max-w-6xl mx-auto relative">
                    <h2 className="text-3xl font-bold text-[#1A2B3C] mb-8">Featured Services</h2>
    ...
    if (services.length === 0) {
        return (
            <div className="bg-[#F8F9FA] py-12 px-4">
                <div className="max-w-6xl mx-auto relative">
                    <h2 className="text-3xl font-bold text-[#1A2B3C] mb-8">Featured Services</h2>
                    <p className="text-gray-500 text-center py-10">No featured services available at the moment.</p>
                </div>
            </div>
        );
    }
